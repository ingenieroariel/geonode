...
try:
    Layer = get_model('layers', 'Layer')
    layers = Layer.objects.filter(table_name__isnull=False).all()
    layers.count()
except AttributeError:
    # Do not run this on initial import
    # (before the layer model is defined)
    pass
except db.utils.DatabaseError:
    # Do not run this on the initial syncdb.
    # (before the database is created).
    pass
else:
    for layer in layers:
        #FIXME: This is a security hole, if you are reading the exec in the line
        # before do not pull this code into core. Instead give Ariel a hard time
        # about security and how model_str can be modified to delete all your files
...
def generate_model(db_key, table_name):
    ...
    #FIXME: See if this variable can be removed.
