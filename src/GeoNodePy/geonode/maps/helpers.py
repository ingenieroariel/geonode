...
from geonode.maps.models import Map, Layer, MapLayer, Contact, ContactRole, Role, get_csw, GeoNodeException
import geoserver
...
def upload(incoming, user=None, overwrite=True):
    ...
    if os.path.isfile(incoming):
        ...
        output.append({'file': incoming, 'name': layer.name})
        return output
        ...
        for short_filename in files:
            ...
            if extension in ['.asc', '.tif', '.shp', '.zip']:
                try:
                    ...
                    logger.info(msg)
                    output.append({'file': filename, 'errors': msg})
                else:
                    output.append({'file': filename, 'name': layer.name})
